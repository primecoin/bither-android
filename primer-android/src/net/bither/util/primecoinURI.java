 ...
 */
public class primecoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(primecoinURI.class);
    ...
    /**
     * Constructs a new primecoinURI from the given string. Can be for any network.
     *
     * @param input The raw URI data to be parsed (see class comments for accepted formats)
     * @throws PrimecoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public primecoinURI(String input) throws PrimecoinURIParseException {
        checkNotNull(input);
...
//            throw new PrimecoinURIParseException("Bad URI syntax", e);
//        }
        ...
        if (input.startsWith("bitcoin://")) {
            ...
            throw new PrimecoinURIParseException("Unsupported URI scheme");
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0) {
            throw new PrimecoinURIParseException("No data found after the bitcoin: prefix");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new PrimecoinURIParseException("Too many question marks in URI");
            }
        }
        ...
        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new PrimecoinURIParseException("No address and no r= parameter found");
        }
    }
    ...
    private void parseParameters(String addressToken, String[] nameValuePairTokens) throws
            PrimecoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1) {
                throw new PrimecoinURIParseException("Malformed Bitcoin URI - no separator in '" +
                        nameValuePairToken + "'");
            }
            if (sepIndex == 0) {
                throw new PrimecoinURIParseException("Malformed Bitcoin URI - empty name '" +
                        nameValuePairToken + "'");
            }
     ...
     */
    private void putWithValidation(String key, Object value) throws PrimecoinURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new PrimecoinURIParseException(String.format("'%s' is duplicated, " +
                    "" + "URI is invalid", key));
        } else {
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("primecoinURI[");
        boolean first = true;
    ...
    public static String convertToBitcoinURI(String address, @Nullable long amount,
        ...
        if (amount != 0) {
            ...
            builder.append(UnitUtil.formatValue(amount, UnitUtil.PrimecoinUnit.XPM));
    ...
    /**
     * <p>Exception to provide the following to {@link primecoinURI}:</p>
     * <ul>
     ...
     */
    public static class PrimecoinURIParseException extends Exception {
        public PrimecoinURIParseException(String s) {
            super(s);
        }

        public PrimecoinURIParseException(String s, Throwable throwable) {
            super(s, throwable);
        }
    }

    public static class OptionalFieldValidationException extends PrimecoinURIParseException {
