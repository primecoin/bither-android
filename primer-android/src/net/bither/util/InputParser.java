    ...
    public abstract static class StringInputParser extends InputParser {
        ...
        public void parse() {
            if (input.startsWith("bitcoin:")) {
                try {
                    final primecoinURI primecoinUri = new primecoinURI(input);
                    final String address = primecoinUri.getAddress();
                    final String addressLabel = primecoinUri.getLabel();
                    final long amount = primecoinUri.getAmount();
                    final String bluetoothMac = (String) primecoinUri.getParameterByName(Bluetooth
                            .MAC_URI_PARAM);
                    bitcoinRequest(address, addressLabel, amount, bluetoothMac);
                } catch (final primecoinURI.PrimecoinURIParseException x) {
