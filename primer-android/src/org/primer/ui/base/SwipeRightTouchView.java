...
import org.primer.util.SdkUtils;
...
public class SwipeRightTouchView extends RelativeLayout {
	...
	public boolean dispatchTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			downLeft = event.getX()
					+ PrimerSetting.SwipeRightGesture.SCROLL_DELAY_HORIZONTAL;
			downTop = event.getY();
			xMove = 0;
			yMove = 0;
			if (toAddShadow) {
		...
		if (event.getAction() == MotionEvent.ACTION_MOVE) {
			...
			if (yMove > -PrimerSetting.SwipeRightGesture.SCROLL_DELAY_VERTICAL
					&& yMove < PrimerSetting.SwipeRightGesture.SCROLL_DELAY_VERTICAL) {
				return true;
			}
			if (showLog) {
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			...
			if (xMove > rlFinanceDetail.getWidth()
					/ PrimerSetting.SwipeRightGesture.DISMISS_DISTANCE_DIVIDER) {
				if (mDragTask != null) {
					post(mDragTask);
				}
				if (showLog) {
