...
import org.primer.runnable.DownloadSpvRunnable;
import org.primer.util.PrimerTimer;
...
public class BlockchainService extends android.app.Service {
    ...
    private PrimerTimer mPrimerTimer;
    ...
    private void scheduleStartBlockchainService(@Nonnull final Context context) {
        PrimerSetting.SyncInterval syncInterval = AppSharedPreference.getInstance().getSyncInterval();
        if (syncInterval == PrimerSetting.SyncInterval.OnlyOpenApp ||
                AddressManager.getInstance().getAllAddresses().size() == 0) {
            return;
        }
        long interval = AlarmManager.INTERVAL_HOUR;
        if (syncInterval == PrimerSetting.SyncInterval.Normal) {
            final long lastUsedAgo = AppSharedPreference.getInstance().getLastUsedAgo();
            if (lastUsedAgo < PrimerSetting.LAST_USAGE_THRESHOLD_JUST_MS) {
                interval = AlarmManager.INTERVAL_FIFTEEN_MINUTES;
                log.info("start INTERVAL_FIFTEEN_MINUTES");
            } else if (lastUsedAgo < PrimerSetting.LAST_USAGE_THRESHOLD_RECENTLY_MS) {
    ...
    public void onDestroy() {
        ...
        if (AppSharedPreference.getInstance().getAppMode() != PrimerjSettings.AppMode.COLD) {
            ...
            if (mPrimerTimer != null) {
                mPrimerTimer.stopTimer();
                mPrimerTimer = null;
            }
    ...
    private final BroadcastReceiver connectivityReceiver = new BroadcastReceiver() {
        ...
        private void check() throws BlockStoreException {
            PrimerjSettings.AppMode mode = AppSharedPreference.getInstance().getAppMode();
    ...
    public void startMarkTimerTask() {
        if (AppSharedPreference.getInstance().getAppMode() != PrimerjSettings.AppMode.COLD) {
            if (mPrimerTimer == null) {
                mPrimerTimer = new PrimerTimer(BlockchainService.this);
                mPrimerTimer.startTimer();
            }
        }
    }
