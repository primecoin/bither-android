...
import org.primer.charts.entity.PrimerOHLCEntity;
...
public class ChartsUtil {
		...
		for (int i = jsonArray.length() - 1; i >= 0; i--) {
			...
			if (volume == 0) {
				if (i == 0) {
					...
					PrimerOHLCEntity primerOHLCEntity = (PrimerOHLCEntity) ohlc
							.get(i - 1);
					open = primerOHLCEntity.getOpen();
					high = primerOHLCEntity.getHigh();
					low = primerOHLCEntity.getLow();
					close = primerOHLCEntity.getClose();
				}

			}
			PrimerOHLCEntity primerOHLCEntity = new PrimerOHLCEntity(open,
					high, low, close, volume, title, time);

			ohlc.add(primerOHLCEntity);
		}
	...
	public synchronized static void initMACandleStickChart(
		...
		MA10.setTitle("MA10");
		MA10.setLineColor(PrimerApplication.mContext.getResources().getColor(
				R.color.ten_kline));
		...
		MA30.setTitle("MA25");
		MA30.setLineColor(PrimerApplication.mContext.getResources().getColor(
	...
	private static List<DateValueEntity> initMA(List<IStickEntity> ohlc,
		...
		for (int i = 0; i < ohlc.size(); i++) {
			float close = (float) ((PrimerOHLCEntity) ohlc.get(i)).getClose();
			if (i < days) {
				sum = sum + close;
				avg = sum / (i + 1f);
			} else {
				sum = sum
						+ close
						- (float) ((PrimerOHLCEntity) ohlc.get(i - days))
								.getClose();
				avg = sum / days;
			}
