...
public class PlaySound {
	...
	public static void loadSound(int soundId) {
		...
		synchronized (sounds) {
			...
			int soundIDOfPool = soundPool.load(PrimerApplication.mContext,
					soundId, 1);
		...
		if (sounds.get(soundId) != null) {
			...
			synchronized (sounds) {
				final int soundIDOfPool = soundPool.load(
						PrimerApplication.mContext, soundId, 1);
				soundPool
	...
	private static void beginPlay(int soundId,
		...
		if (soundId == 0) {
			...
			AudioManager mgr = (AudioManager) PrimerApplication.mContext
