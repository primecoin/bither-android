...
package org.primer.activity.cold;
...
import org.primer.ui.base.dialog.DialogHdmImportWordListReplace;
...
public class HdmImportWordListActivity extends SwipeRightFragmentActivity implements TextView
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (extra != null && extra.containsKey(PrimerSetting.INTENT_REF.IMPORT_HD_SEED_TYPE)) {
            importHDSeedType = (ImportHDSeed.ImportHDSeedType) extra.getSerializable(PrimerSetting.INTENT_REF.IMPORT_HD_SEED_TYPE);
        } else if (extra != null && extra.containsKey(PrimerSetting.INTENT_REF.IMPORT_HDM_SEED_TYPE)) {
            importHDSeedType = (ImportHDSeed.ImportHDSeedType) extra.getSerializable(PrimerSetting.INTENT_REF.IMPORT_HDM_SEED_TYPE);
        }
        initView();
    ...
    private void isZhTw() {
        try {
            ...
            for (String word: words) {
                mnemonic = MnemonicCode.instanceForWord(new MnemonicCodeAndroid(), word);
                ArrayList<String> zhCnWorsList = mnemonic.getWordListForInputStream(PrimerApplication.mContext.getResources().openRawResource(R.raw.mnemonic_wordlist_zh_cn));
                ArrayList<String> zhTwWorsList = mnemonic.getWordListForInputStream(PrimerApplication.mContext.getResources().openRawResource(R.raw.mnemonic_wordlist_zh_tw));
