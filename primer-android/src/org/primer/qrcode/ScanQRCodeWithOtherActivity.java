...
import org.primer.util.PlaySound;

public class ScanQRCodeWithOtherActivity extends ScanActivity {
    private TextView tv;
    private TextView tvTitle;
    private PrimerSetting.QRCodeType qrCodeType;
    private String lastResult;

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getIntent() != null && getIntent().hasExtra(PrimerSetting.INTENT_REF.QRCODE_TYPE)) {
            qrCodeType = (PrimerSetting.QRCodeType)
                    getIntent().getSerializableExtra(PrimerSetting.INTENT_REF.QRCODE_TYPE);
        }
        if (qrCodeType == null) {
            finish();
        }
