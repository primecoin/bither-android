...
import org.primer.fragment.QrCodeFragment;
import org.primer.fragment.QrCodeFragment.QrCodeFragmentDelegate;
import org.primer.ui.base.DropdownMessage;
import org.primer.ui.base.SwipeRightFragmentActivity;
import org.primer.ui.base.dialog.DialogQRCodeOption;
import org.primer.ui.base.listener.IBackClickListener;

import java.util.ArrayList;
import java.util.List;

public class PrimerQRCodeActivity extends SwipeRightFragmentActivity implements DialogQRCodeOption.ISwitchQRCode {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null && intent.getExtras() != null) {
            if (intent.getExtras().containsKey(PrimerSetting.INTENT_REF.QR_CODE_STRING)) {
                codeString = intent.getExtras().getString(PrimerSetting.INTENT_REF.QR_CODE_STRING);
            }
            if (intent.getExtras().containsKey(PrimerSetting.INTENT_REF.OLD_QR_CODE_STRING)) {
                oldString = intent.getExtras().getString(PrimerSetting.INTENT_REF.OLD_QR_CODE_STRING);
            }
            if (intent.getExtras().containsKey(PrimerSetting.INTENT_REF.QR_CODE_HAS_CHANGE_ADDRESS_STRING)) {
                hasChangeAddress = intent.getExtras().getBoolean(PrimerSetting.INTENT_REF.QR_CODE_HAS_CHANGE_ADDRESS_STRING);
            }
        }
    ...
    private void refresh() {
        ...
        if (isNewVerion) {
            ...
            if (!firestIn) {
                DropdownMessage.showDropdownMessage(PrimerQRCodeActivity.this, R.string.use_new_version_qrcode);
            }
        } else {
            pageContentList.clear();
            ...
            if (!firestIn) {
                DropdownMessage.showDropdownMessage(PrimerQRCodeActivity.this, R.string.use_old_version_qrcode);
            }
        }
    ...
    private View.OnClickListener switchClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (hasChangeAddress) {
                DropdownMessage.showDropdownMessage(PrimerQRCodeActivity.this, R.string.old_version_no_support_change_adress);
            } else {
                new DialogQRCodeOption(PrimerQRCodeActivity.this, PrimerQRCodeActivity.this).show();
            }
        }
    };
    ...
    protected String getTitleString() {
        ...
        Intent intent = getIntent();
        if (intent != null && intent.getExtras() != null) {
            title = intent.getExtras().getString(PrimerSetting.INTENT_REF.TITLE_STRING);
        }
        return title;
    }
